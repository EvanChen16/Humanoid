...
class ImageSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.building_model_srv = self.create_service(BuildModel, '/BuildModel', self.building_model_callback)
...
##############################################################
    ...
    def hsv_value_callback(self, msg):
        self.hmin = round(float(msg.hmin)/2)
        self.hmax = round(float(msg.hmax)/2)
    ...
    def color_model_callback(self, msg):
        self.building_model = bool(msg.buildingmodel)

    def building_model_callback(self, request, response):
        self.get_logger().info(f"Received request to build model: {request.build}")
        if request.build:
            response.already = True
            self.building = True
    ...
    def adjust_image_callback(self, request, response):
        ...
        response.hmin = round(float(self.hsvcolorraberdict[self.colorlabel_str]['lower'][0])*2)
        response.hmax = round(float(self.hsvcolorraberdict[self.colorlabel_str]['upper'][0])*2)
    ...
    def image_callback(self,msg):
        ...
        # Create a new image where each pixel is black
        new_image = np.zeros_like(cv_image)

        # Define color_mapping before it's used
        color_mapping = {
        ...
        # If 'building' is True, overlay all models on the new image
        if self.building:
            mask_total = np.zeros_like(hsv[:,:,0])
            for color_label in self.color_ranges:
            ...
            for color_label in self.color_ranges:
                ...
                upper = self.color_ranges[color_label]['upper']
                mask_color = cv2.inRange(hsv, lower, upper)
                mask_color = cv2.bitwise_and(mask_color, mask_total)
                new_color = color_mapping[int(color_label)]
                new_image[mask_color != 0] = new_color

            # Apply median blur to reduce noise
            new_image = cv2.medianBlur(new_image, 5)

        # If 'building_model' is True, publish the new image
        if not self.building_model:
            image_message = self.bridge.cv2_to_imgmsg(new_image, encoding="bgr8")
